...
class _ToolboxScreenState extends State<ToolboxScreen> {
  ...
  Future<void> _handleDonateTap(BuildContext context) async {
    ...
    showDialog(context: context, barrierDismissible: false, builder: (_) => const Center(child: Card(child: Padding(padding: EdgeInsets.all(32), child: CircularProgressIndicator()))));

    await Future.delayed(const Duration(seconds: 1));

    final prefs = await SharedPreferences.getInstance();
    int counter = (prefs.getInt('donation_counter') ?? 0) + 1;
